...
public class AddBuilding extends AppCompatActivity {
    ...
    private int pos = 1;
    private String installed_equip,installedid;
    ...
    public boolean validate(){
        if(build_name_et.getText().length()==0 || select_build_type.getText().length()==0 ||
                select_installed_equip.getText().length()==0){

            AlertDialog.Builder builder1 = new AlertDialog.Builder(AddBuilding.this);
    ...
    public void insertData(){
        try {
            ...
            long added =  dataBaseHelper.insertData1(dataBaseHelper,build_name, build_address, build_zip, build_city, build_type_id,
                    "NA", "NA", installedid, build_type_SqFoot, build_agency_id, build_year);
            if(added!=-1){

                int buildingid = pos;
                if(utils.getdata("pos")!=null && !utils.getdata("pos").equals("")){
                ...
                switch (requestCode){
                    ...
                    case ApplicationConstant.building_installed_equipment_constant:{
                        try{
                            ...
                            installedid = installed_equip;
                            installed_equip = installed_equip+" out of "+total;
                           //String installed_equip_total = installed_equip+" out of "+total;
                            select_installed_equip.setText(installed_equip);
                        }catch (Exception e){
                            e.printStackTrace();
                        }
                        break;
